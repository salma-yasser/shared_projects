<div class="modal fade" id="add_program" aria-hidden="true" role="dialog">
    <div class="modal-dialog modal-dialog-centered modal modal-lg" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">إعادة البرنامج</h5>
        ...
        <div class="modal-body">
            <div class="row">
                <span class="col-md-12">نماذج البرنامج</span>
                <div id="templates" class="col-md-12 pb-3">

                </div>
            </div>
          <form id="add_program_form" action="{{ route('child_programs.store') }}" method="POST" enctype="multipart/form-data">
              @csrf
            <input id="child_department" name="child_department" value="{{ $child_patient->id }}" hidden/>
            <input name="type" value="add_program" hidden/>
            <div class="row form-row">
                <div class="col-md-12">
                    <div class="form-group">
                        <label class="form-control-label" for="file_path">رفع الملفات<span class="text-danger">*</span></label>
                        <div class="needsclick dropzone" id="program-dropzone">
                        </div>
                    </div>
                </div>
            </div>
...
<script>
if ("{{count($errors->add_program) > 0}}"){
    $('#add_program').modal('show');
}

$(document).ready(function() {
    $('#add_program').on('show.bs.modal', function(e) {
        var child_department = $(e.relatedTarget).data('child-department');
        $("#add_program #child_department").val(JSON.stringify(child_department));
        ...
        $('#add_program #templates').html(templates);
    });

  $("#add_program #add_program_form").submit(function(e){
      $("#add_program #text-btn").attr("hidden", true);
      $("#add_program #text-loading").attr("hidden", false);
      $("#add_program #save").attr("disabled", true);
      return true;
    });
  });

var uploadedFileMap = {}
Dropzone.options.programDropzone = {
    ...
    success: function (file, response) {
        $('form').append('<input type="hidden" name="program_document[]" value="' + response.name + '">')
    ...
    removedfile: function (file) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('form').find('input[name="program_document[]"][value="' + name + '"]').remove()
            }
        });
    },
    init: function init() {
