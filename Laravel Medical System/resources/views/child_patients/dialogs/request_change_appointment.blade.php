<div class="modal fade" id="request_change_appointment" aria-hidden="true" role="dialog">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">طلب تعديل موعد</h5>
        ...
        <div class="modal-body">
          <form id="form" method="POST" enctype="multipart/form-data">
            ...
            <input name="type" value="change_appointment" hidden/>
            ...
            <!-- <button type="submit" class="btn btn-primary btn-block" disabled="true">تأكيد</button> -->
          </form>
        </div>
      </div>
    </div>
...
if ("{{count($errors->request_extra_rebook_appointment) > 0}}"){
    $('#request_change_appointment').modal('show');
}

function moveNextChangeAppointment(index){
  $("#request_change_appointment #schedule_div_"+(index-1)).attr('hidden',true);
  $("#request_change_appointment #schedule_div_"+index).attr('hidden',false);
  return false;
}
function movePreviousChangeAppointment(index){
  $("#request_change_appointment #schedule_div_"+(index+1)).attr('hidden',true);
  $("#request_change_appointment #schedule_div_"+index).attr('hidden',false);
  return false;
}

function addRequestChangeAppointment(object, start_date){
    ...
    if(appointments.length<max){
      ...
      if(appointments.length==(max-1)){
        $("#request_change_appointment button[type='submit']").attr("disabled", false);
      }
    }else{
...
$(document).ready(function () {
  ...
  $('#request_change_appointment').on('show.bs.modal', function(e) {
    ...
    var session_duration = $(e.relatedTarget).data('session_duration');
      ...
      $("#request_change_appointment form").attr("action", url);
      $("#request_change_appointment #child_appointment").val(JSON.stringify(appointment));
    // $("#request_change_appointment #child_appointment_id").val(appointment.id);
    $("#request_change_appointment #schedule_div").empty();
    ...
    $.ajax({
      ...
      data: {
       ...
       beforeSend: function(){
         $("#request_change_appointment #overlay").fadeIn(300);
       },
       complete: function(){
           setTimeout(function(){
           $("#request_change_appointment #overlay").fadeOut(300);
         },500);
       },
       error: function (data) {
       ...
       success: function (data) {
         ...
         for(var index=0; index<Math.ceil(expire_duration/7); index++){
           $("#request_change_appointment #schedule_div").append('<div class="card booking-schedule schedule-widget" id="schedule_div_'+index+'" '+(index!=0?'hidden':'')+'>'
           +'	<!-- Schedule Header -->'
           ...
           $("#request_change_appointment #day_slot_"+index+" ul").append('<li class="left-arrow" '+(index==0?'hidden':'')+'>'
           +'					<a href="" onclick="return movePreviousChangeAppointment('+(index-1)+');">'
           ...
           while(day<6 && start_date.isBefore(expire_date.add(1, 'days'))){
             if(start_date.format("ddd")=='Fri'){
               ...
               //Header
               $("#request_change_appointment #day_slot_"+index+" ul").append('<li>'
                 ...
                 if(doctor_sessions[doctor_session].type == 3){
                   ...
                   for(var i=0; i<session_sc.length; i++){
                     if(eval('session_sc['+i+'].day_'+day_num) != null){
                       ...
                       day_sessions += '	<a id="date_'+date+'" class="timing" href="" onclick="return addRequestChangeAppointment(this,'+start_date+')">'
               ...
               $("#request_change_appointment #time_slot_"+index+" ul").append('<li>'
           ...
           $("#request_change_appointment #day_slot_"+index+" ul").append('<li class="right-arrow" '+(index==(Math.ceil(expire_duration/7)-1)?'hidden':'')+'>'
           +'					<a href="" onclick="return moveNextChangeAppointment('+(index+1)+');">'
         ...
         for(var i = 0; i<doctor_appointments.length; i++){
           $($("#request_change_appointment a#date_"+((doctor_appointments[i].time).replace(/ /gi, "_")).replace(/:/gi, "-")+".timing").not(".disabled")[0]).addClass("disabled");
         }
       }
      });
  });

  $("#request_change_appointment form").submit(function (e) {
      $("#request_change_appointment #text-btn").attr("hidden", true);
      $("#request_change_appointment #text-loading").attr("hidden", false);
      $("#request_change_appointment #save").attr("disabled", true);
      return true;
  });
});
</script>
