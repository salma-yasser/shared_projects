...
@section('header')
    <div class="page-header">
        <h1><i class="glyphicon glyphicon-edit"></i> CourseMenus / Edit #{{$course_menu->id}}</h1>
    </div>
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{ route('course_menus.update', $course_menu->id) }}" method="POST">
                <input type="hidden" name="_method" value="PUT">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">

                <div class="form-group @if($errors->has('menu_id')) has-error @endif">
                       <label for="menu_id-field">Menu_id</label>
                    <input type="text" id="menu_id-field" name="menu_id" class="form-control" value="{{ is_null(old("menu_id")) ? $course_menu->menu_id : old("menu_id") }}"/>
                    ...
                    <input type="text" id="course_id-field" name="course_id" class="form-control" value="{{ is_null(old("course_id")) ? $course_menu->course_id : old("course_id") }}"/>
